...
CREATE OR REPLACE VIEW VER_VENDAS AS
	SELECT V.COD as venda, PROD.NOME as produto, IV.QNTD as quantidadde, (IV.VALOR / IV.QNTD) as valor_unitario, 
...
--ADD_VENDA(CPF_C, NOME_P, QNTD_V, CPF_F);
...
CREATE OR REPLACE FUNCTION GERAR_NOTA_VENDA_GERAL(COD_V INT)
RETURNS TABLE(
	venda int,
	nome_produto varchar(100),
	quantidade int,
...
BEGIN
	...
	IF(NOME_TS ILIKE 'BANHO')THEN
		SELECT NOME INTO CARGO_F FROM CARGO WHERE NOME ILIKE 'BANHISTA';
	ELSIF(NOME_TS ILIKE 'TOSA')THEN
		SELECT NOME INTO CARGO_F FROM CARGO WHERE NOME ILIKE 'TOSADOR';
	END IF;
	
	IF EXISTS(SELECT * FROM TIPO_SERVICO WHERE COD = COD_T) THEN
		IF EXISTS (SELECT * FROM FUNCIONARIO F JOIN CARGO CAR ON CAR.COD = F.COD_CARGO WHERE F.COD = COD_F AND CAR.NOME ILIKE CARGO_F) THEN
			IF EXISTS(SELECT * FROM CLIENTE WHERE CPF = CPF_C)THEN
...
CREATE OR REPLACE FUNCTION GERAR_NOTA_SERV_GERAL(COD_S INT)
RETURNS TABLE(
	servico int,
	tipo_servico varchar(100),
	valor_servico numeric(8, 2),
...
DECLARE
    ...
    CARGO_COD INT;
BEGIN
    SELECT COD_CARGO INTO CARGO_COD
    FROM FUNCIONARIO
    WHERE COD = FUNCIONARIO_ID;

    IF CARGO_COD = 1 THEN -- ATENDENTE/VENDEDOR
    ...
    ELSIF CARGO_COD = 2 -- BANHISTA
		THEN COMISSAO := COALESCE(SUM(VALOR * 0.07), 0)
        FROM SERVICO
        WHERE COD_FUNC = FUNCIONARIO_ID
            AND DT_HORA BETWEEN DATA_INICIO AND DATA_FIM
            AND PAGO = TRUE;
    ...
    ELSE
        COMISSAO := 0; -- CASO NÃO SEJA ATENDENTE NEM BANHISTA
...
BEGIN
    ...
    ELSIF (CARGO ILIKE 'BANHISTA' OR CARGO ILIKE 'TOSADOR')
		THEN COMISSAO := COALESCE(SUM(VALOR * 0.07), 0)
        FROM SERVICO
        WHERE COD_FUNC = FUNCIONARIO_ID
            AND DT_HORA BETWEEN DATA_INICIO AND DATA_FIM
	...
	ELSIF (CARGO ILIKE 'GERENTE' OR CARGO ILIKE 'SUPERVISOR') THEN
		RAISE EXCEPTION 'ESTE CARGO NÃO POSSUI COMISSÃO!';
    ELSE
        RAISE EXCEPTION 'CARGO NÃO ENCONTRADO!';
    END IF;
